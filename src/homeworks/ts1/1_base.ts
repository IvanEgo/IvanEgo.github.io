...
export const getNumberedArray = <T,>(arr: Array<T>) => arr.map((value: T, number: number) => ({ value, number }));
export const toStringArray = <T,>(arr: Array<T>): Array<string> => arr.map((value: T, number: number) => `${value}_${number}`);
